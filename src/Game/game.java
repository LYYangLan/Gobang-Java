...
public class game extends JFrame{
	...
	private void init() {
		// 窗体的常用设置
		this.setSize(760, 780);// 设置窗体的大小，宽度和高度，单位是像素
...
class PicJpanel extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		//try {
		/********实现适应窗口的背景图片插入********/
			ImageIcon bi  =new ImageIcon("F:\\Monkey\\JavaWorkspace\\Gobang\\timg.jpg");
			bi.setImage(bi.getImage().getScaledInstance(mj.getWidth(),mj.getHeight(),Image.SCALE_AREA_AVERAGING));//这里设置图片的大小和窗口的大小相等
			//BufferedImage bi = ImageIO.read(new File("F:\\Monkey\\JavaWorkspace\\Gobang\\timg.jpg"));
			g.drawImage(bi.getImage(), 0, 0, this);
		//} catch (IOException e) {
			//e.printStackTrace();
		//}
